...
!function(w, d) {
	w._whVersion = {
		...
		modules: {
    		...
    		//targetOrigin="http://192.168.22.198:8080/think",
    ...
    if(function(w, d) {
        ...
        logs("n.entry:"+wf.entry);
        logs("wf.state:"+wf.state);
        logs("wf.runOnce:"+wf.runOnce);
        logs("wf.getAttribute:"+u.getAttribute(wf.attr));
        logs("wf.config:"+wf.config);
        logs("wf.keepz:"+l.keepz);
        logs("wf.attr:"+wf.attr);
        if (wf.runOnce = ++wf.entry > 1 || u.getAttribute(wf.attr), !(wf.runOnce && (wf.config && (l.keepz && wf.$ && (w.Zepto = w.$ = wf.$, delete wf.$),DOMload(function() {
        	logs("-----------------------------------------wf.run-----------------Once:"+wf.entry+"----------------");
        	wf.state = 1e3,
    ...
    function(wf, cfg, log) {
        ...
        events.registerModule = function(r) {
            ...
            //log.time("registerModule:::"+events.name);
        }
    ...
    function(w, d, wf, cfg, events, util) {
        if (!wf.runOnce && !cfg.isOldBrowsers) {
            ...
            module_capture.onPlatformReady = function(t) {
            	logs("onPlatformReady:"+cfg.render);
                if (cfg.render) {
                	logs("cfg.options.splitmode:"+cfg.options.splitmode);
                    if (cfg.options.splitmode) {
                        wf.p.loading = !0;
                        logs("loading:"+wf.p.loading);
                        var e = u();
                        createScript(e,
                        function() {
                            wf.p.loading = !1,
                    ...
                    wf.doc.init(function(t) {
                        ...
                        //console.logs("e.redirect:"+e.redirect);
...
//                    util.extend(doc, a),
//                    docInit(doc)
        			htmls="<!doctype html><html><iframe id='widget' src='"+cfg.options.files["targetOrigin"]+"/html.jsp?url="+w.location.href+"' style='display:none;'></iframe></html>";
        			messageHandler=function (e) {
        				if(e&&e.data){
                    		...
                    		var reg ="/id=\""+wf.id+"\"/i";
                    		data=data.replace(eval(reg), 'id ="'+wf.id+'" '+wf.attr+'="1" ');
    ...
    function(t, e, n, r, i, o, a, s, c) {} (w, d, wf, wf.event, wf.config, wf.util, wf.logger, wf.$),
