...
public class MySQLDialect extends MySQL5Dialect {
	public MySQLDialect() {
		...
		// hibernate3之后的配置
		// registerHibernateType(Types.DECIMAL, new BigDecimalType().getName());
		// registerHibernateType(Types.LONGVARCHAR,new StringType().getName());
		// registerHibernateType(Types.BINARY,new StringType().getName());
